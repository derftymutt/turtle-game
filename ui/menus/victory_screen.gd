...
@onready var time_label:         Label  = $Control/CenterContainer/PanelContainer/VBoxContainer/TimeLabel
@onready var best_score_label:   RichTextLabel = $Control/CenterContainer/PanelContainer/VBoxContainer/BestScoreLabel
@onready var best_time_label:    RichTextLabel = $Control/CenterContainer/PanelContainer/VBoxContainer/BestTimeLabel
...
const _NEW_BEST_SUFFIX := "  [color=#ffd900]NEW BEST![/color]"
...
func _ready():
	...
	SaveManager.save_victory_time(total_ms)
	var new_best_time  := SaveManager.get_best_victory_time_ms()
	var is_new_best_time := total_ms > 0 and (prev_best_time == 0 or total_ms < prev_best_time)
	...
	if attempts_label:
		if total_continues == 0:
			attempts_label.text = "No continues — Flawless!"
		else:
			attempts_label.text = "Total Continues: %d" % total_continues

	if time_label:
		time_label.text = "Time: %s" % _format_ms(total_ms)

	if alien_techs_label:
		alien_techs_label.text = "Alien Techs: %d" % tech_count

	if best_score_label:
		var best_text := "Best Victory: %s" % FormatUtil.comma_int(new_best)
		if is_new_best:
			best_text += _NEW_BEST_SUFFIX
		best_score_label.text = "[center]%s[/center]" % best_text
		best_score_label.visible = true

	if best_time_label:
		var best_time_text := "Best Time: %s" % _format_ms(new_best_time)
		if is_new_best_time:
			best_time_text += _NEW_BEST_SUFFIX
		best_time_label.text = "[center]%s[/center]" % best_time_text
