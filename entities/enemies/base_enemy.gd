...
func _on_damage_area_entered(body: Node2D):
	if body.is_in_group("player") and body.has_method("take_damage"):
		if body not in _contact_players:
			_contact_players.append(body)
		if current_health > 0 and _contact_damage_ready:
			_deal_damage_to_player(body)

func _on_damage_area_exited(body: Node2D):
...
## Multi-Beam shocks these instead of damaging them: permanently invincible
## enemies (sea urchin). The boss submarine is excluded — it's invincible by
## design (it needs super-speed hits) and mustn't be lockable at will.
func can_be_shocked() -> bool:
	return is_invincible and not is_in_group("submarine_boss")

## Multi-Beam shock — frozen and harmless for `duration` seconds. See EnemyShock.
